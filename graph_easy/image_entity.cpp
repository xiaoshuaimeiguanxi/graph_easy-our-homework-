...
bool ImageEntity::load_from_file(const char* path)
{
	...
	if (!surface)
	{
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, u8"³ÌÐòÆô¶¯Ê§°Ü", u8"¼ÓÔØÍ¼ÏñÊ§°Ü", window);
	}
	update_texture();
	return true;
}
...
void ImageEntity::update_texture()
{
    ...
    if (!converted) {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, u8"³ÌÐòÆô¶¯Ê§°Ü", u8"×ª»»Í¼Ïñ¸ñÊ½Ê§°Ü", window);
    }
